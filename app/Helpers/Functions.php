...
if (!function_exists('generate_user_code')) {
    function generate_user_code($user_name)
    {

        $user_initial = substr($user_name, 0, 1);

        $users = \App\Models\Admin::where('code', 'like', $user_initial . '%')->get();

        if ($users) {
            $count = count($users) + 1;
            $user_initial =  $user_initial . $count;
        }
        return strtoupper($user_initial);
    }
}
